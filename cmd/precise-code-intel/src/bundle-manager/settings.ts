...
/** How many times to retry requests to precise-code-intel-server in the background. */
export const MAX_REQUEST_RETRIES = readEnvInt('MAX_REQUEST_RETRIES', 60)

/** How long to wait (minimum, in seconds) between precise-code-intel-server request attempts. */
export const MIN_REQUEST_RETRY_TIMEOUT = readEnvInt('MIN_REQUEST_RETRY_TIMEOUT', 1)

/** How long to wait (maximum, in seconds) between precise-code-intel-server request attempts. */
